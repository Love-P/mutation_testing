from src import Polynomial  # Import the Polynomial class from your module
from mutation import MutatedPolynomialWrapper
...
def test_operations_with_zero_polynomial():
    ...
    assert result_add.coefficients == poly.coefficients, "Adding zero should not change the polynomial"

    # Subtracting zero should not change the polynomial
    result_sub = poly - zero_poly
    assert result_sub.coefficients == poly.coefficients, "Subtracting zero should not change the polynomial"

    # Multiplying by zero should result in a zero polynomial
    result_mul = poly * zero_poly
    assert result_mul.coefficients == [0 for _ in range(len(poly.coefficients) + len(zero_poly.coefficients) - 1)], "Multiplying by zero should result in a zero polynomial"
...
def test_mutated_mul_red_code():
    ...
    poly_product = poly1 * poly2
    assert poly_product.coefficients != [3, -3, 2, -2]  # Mutated multiplication with redundant code
...
def test_mutated_add_sub():
    ...
    poly_sum = poly1 + poly2
    poly_diff = poly1 - poly2
    assert poly_sum.coefficients != [3, 1, 1] and poly_diff.coefficients != [3, -1, 3], "Mutated addition or subtraction not detected"
...
def test_mutated_str():
    mutated_poly = MutatedPolynomialWrapper([3, 0, 2])
    assert str(mutated_poly) != "3x^2 + 2", "Mutated __str__ representation not detected"
...
def test_mutated_mul_aor():
    ...
    poly_product = poly1 * poly2
    assert poly_product.coefficients != [3, -2, 2, -2]  # Mutated multiplication with AOR
...
def test_mutated_mul_altered_coeff():
    ...
    poly_product = poly1 * poly2
    assert poly_product.coefficients != [3, 1, 1]  # Mutated multiplication with altered coefficients
